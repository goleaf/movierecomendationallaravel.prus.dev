...
class TrendingService
{
    ...
    public function snapshot(int $days, int $limit = 8): Collection
    {
        ...
        $cacheKey = implode(':', ['trending-snapshot', $days, $limit]);

        return AnalyticsCache::rememberTrends($cacheKey, function () use ($days, $limit): Collection {
    ...
    public function filtered(int $days, string $type = '', string $genre = '', ?int $yearFrom = null, ?int $yearTo = null, int $limit = 40): Collection
    {
        ...
        return AnalyticsCache::rememberTrends($cacheKey, function () use ($days, $type, $genre, $yearFrom, $yearTo, $limit): Collection {
            [$from, $to] = $this->timeframe($days);
