...
class SsrMetricsService
{
    ...
    public function buildPayload(string $path, int $firstByteMs, array $metrics, ?int $score = null): array
    {
        ...
        $htmlBytes = $metrics['html_bytes'] ?? $metrics['html_size'] ?? null;
        $htmlBytes = $htmlBytes !== null ? (int) $htmlBytes : null;

        $movieId = null;

        if (isset($metrics['movie_id']) && is_numeric($metrics['movie_id'])) {
            $candidate = (int) $metrics['movie_id'];
            $movieId = $candidate > 0 ? $candidate : null;
        }
        ...
        $metaPayload = array_merge(
            is_array($metrics['meta'] ?? null) ? $metrics['meta'] : [],
            [
                'first_byte_ms' => $firstByteMs,
                'html_size' => $metrics['html_size'] ?? ($htmlBytes ?? 0),
        ...
        $payload['score'] = $score;
        $payload['html_bytes'] = $htmlBytes;
        $payload['collected_at'] = $recordedAtIso;
        $payload['recorded_at'] = $recordedAtIso;
        $payload['first_byte_ms'] = $firstByteMs;
