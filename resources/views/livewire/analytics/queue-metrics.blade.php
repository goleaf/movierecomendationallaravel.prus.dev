    ...
    <div class="bg-white shadow-sm rounded-xl p-6 space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <div class="rounded-lg border border-gray-200 p-4">
                <div class="text-sm font-medium text-gray-600">{{ __('admin.metrics.queue_label') }}</div>
                <div class="mt-2 text-2xl font-semibold text-gray-900">{{ number_format($metrics['queue']) }}</div>
            </div>
            <div class="rounded-lg border border-gray-200 p-4">
                <div class="text-sm font-medium text-gray-600">{{ __('admin.metrics.failed_label') }}</div>
                <div class="mt-2 text-2xl font-semibold text-gray-900">{{ number_format($metrics['failed']) }}</div>
            </div>
            <div class="rounded-lg border border-gray-200 p-4">
                <div class="text-sm font-medium text-gray-600">{{ __('admin.metrics.processed_label') }}</div>
        ...
        @if($metrics['horizon']['workload'] || $metrics['horizon']['supervisors'])
            <div class="space-y-4">
                @if($metrics['horizon']['workload'])
                    <div>
                        <h4 class="text-sm font-medium text-gray-600">{{ __('admin.metrics.horizon_workload') }}</h4>
                ...
                @if($metrics['horizon']['supervisors'])
                    <div>
                        <h4 class="text-sm font-medium text-gray-600">{{ __('admin.metrics.horizon_supervisors') }}</h4>
        ...
        @else
            <p class="text-sm text-gray-500">{{ __('admin.metrics.horizon_empty') }}</p>
        @endif
    </div>
</div>
