        ...
        <div class="flex flex-col gap-4">
            <form wire:submit.prevent="apply" @class([
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-gray-600" for="trends-days">{{ __('admin.trends.days_label') }}</label>
                ...
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-gray-600" for="trends-type">{{ __('admin.trends.type_placeholder') }}</label>
                ...
                @if($showAdvanced)
                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-gray-600" for="trends-genre">{{ __('admin.trends.genre_placeholder') }}</label>
                        <input id="trends-genre" type="text" wire:model.defer="filters.genre" class="fi-input block w-full rounded-lg border-gray-300" />
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-gray-600" for="trends-year-from">{{ __('admin.trends.year_from_placeholder') }}</label>
                    ...
                    <div class="flex flex-col gap-1">
                        <label class="text-sm font-medium text-gray-600" for="trends-year-to">{{ __('admin.trends.year_to_placeholder') }}</label>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                @foreach($items as $item)
                    <a href="{{ route('movies.show', ['movie' => $item['id']]) }}" class="flex flex-col overflow-hidden rounded-xl border border-gray-200 transition hover:border-primary-500">
                        @if($item['poster_url'])
                            <img src="{{ $item['poster_url'] }}" alt="{{ $item['title'] }}" loading="lazy" class="aspect-[2/3] w-full object-cover" />
                        @endif
