        ...
        @else
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                @foreach($items as $item)
                    @php($posterUrl = artwork_url($item['poster_url'] ?? null))
                    <a href="{{ route('movies.show', ['movie' => $item['id']]) }}" class="flex flex-col overflow-hidden rounded-xl border border-gray-200 transition hover:border-primary-500">
                        @if($posterUrl)
                            <img src="{{ $posterUrl }}" alt="{{ !empty($item['title']) ? 'Постер фильма «' . $item['title'] . '»' : 'Постер фильма' }}" loading="lazy" class="aspect-[2/3] w-full object-cover" />
                        @endif
